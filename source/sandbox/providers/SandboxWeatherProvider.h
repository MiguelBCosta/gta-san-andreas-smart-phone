...
private:
    int currentWeatherId = 1; // Default Sunny
    int nextWeatherId = 4;    // Default Cloudy
...
public:
    WeatherInfo GetCurrentWeather() override {
        return MapWeather(currentWeatherId);
    }

    WeatherInfo GetNextWeather() override {
        return MapWeather(nextWeatherId);
    }
    ...
    void SetRainIntensity(float intensity) {
        ...
        if (rainIntensity < 0.0f) rainIntensity = 0.0f;
        if (rainIntensity > 1.0f) rainIntensity = 1.0f;
    }

    int GetCurrentWeatherId() const { return currentWeatherId; }
    int GetNextWeatherId() const { return nextWeatherId; }
};
