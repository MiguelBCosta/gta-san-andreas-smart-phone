...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  WNDCLASSEX wc = {
      ...
      _T("MoonPhone Sandbox"),  nullptr};
  ::RegisterClassEx(&wc);
  HWND hwnd = ::CreateWindow(wc.lpszClassName, _T("MoonPhone UI Sandbox"),
  ...
  while (!done) {
    ...
    ImGui::Begin("Controles do Clima (Sandbox)", nullptr,
    ...
    const char *climates[] = {"0: Muito ensolarado (LS)",
                              "1: Ensolarado (LS)",
    ...
    if (ImGui::Combo("Clima Atual", &currentId, climates,
                     IM_ARRAYSIZE(climates))) {
      sandboxWeather.SetCurrentWeather(currentId);
    }

    // 2. Proximo clima combo
    int nextId = sandboxWeather.GetNextWeatherId();
    if (ImGui::Combo("Proximo Clima", &nextId, climates,
                     IM_ARRAYSIZE(climates))) {
    ...
    ImGui::Begin("Controles de Persistencia (Sandbox)", nullptr,
                 ImGuiWindowFlags_AlwaysAutoResize);
    ...
    ImGui::SetNextWindowSize(ImVec2(360, 160), ImGuiCond_FirstUseEver);
    ImGui::Begin("Controles de Ligacoes (Sandbox)", nullptr,
                 ImGuiWindowFlags_AlwaysAutoResize);
    ...
    const char *callers[] = {"sweet",     "ryder",     "cesar",      "catalina",
                             "woozie",    "truth",     "toreno",     "crash",
                             "zero",      "denise",    "michelle",   "helena",
                             "katie",     "barbara",   "millie",     "kendl",
                             "smoke",     "ogloc",     "jethro",     "kentpaul",
                             "rosenberg", "salvatore", "girlfriend", "unknown"};
    ImGui::Combo("Remetente", &selectedCallerIdx, callers,
                 IM_ARRAYSIZE(callers));
    ...
    ImGui::SetNextWindowSize(ImVec2(360, 160), ImGuiCond_FirstUseEver);
    ImGui::Begin("Controles de SMS (Sandbox)", nullptr,
                 ImGuiWindowFlags_AlwaysAutoResize);
    ...
    const char *missions[] = {"ryder1 (Ryder)",
    ...
    const char *missionIds[] = {
        "ryder1", "ogloc2", "sweet3",  "smoke4", "sweet8",  "ryder4",
        "sweet9", "crash2", "sweet12", "cesar2", "crash3",  "truth1",
        "syn2",   "steal3", "steal4",  "syn7",   "toreno4", "rose1",
        "salv2",  "heist5", "sweet19"};
    ImGui::Combo("Missao", &selectedMissionIdx, missions,
                 IM_ARRAYSIZE(missions));
    ...
    ImGui::Begin("Controles da Garagem (Sandbox)", nullptr,
                 ImGuiWindowFlags_AlwaysAutoResize);
    ...
    if (deliveries.empty()) {
      ...
      for (size_t i = 0; i < deliveries.size(); i++) {
        ...
        ImGui::Text("ID: %d | Distancia: %.1fm %s", del.id, del.distance,
                    del.delivered ? "(Entregue)" : "(A caminho)");
        if (del.delivered) {
          ImGui::SameLine();
    ...
    ImGui::Begin("Controles do Mapa (Sandbox)", nullptr,
                 ImGuiWindowFlags_AlwaysAutoResize);
    ...
    if (sandboxMap.m_waypointActive) {
      ...
      ImGui::TextColored(ImVec4(1.0f, 0.2f, 0.2f, 1.0f),
                         "Waypoint Ativo: (%.1f, %.1f)", sandboxMap.m_waypointX,
                         sandboxMap.m_waypointY);
