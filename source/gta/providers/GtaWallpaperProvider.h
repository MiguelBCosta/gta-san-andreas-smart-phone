...
class GtaWallpaperProvider : public IWallpaperProvider {
private:
...
public:
    GtaWallpaperProvider(IDirect3DDevice9* device) : m_device(device) {}
    
    ~GtaWallpaperProvider() {
        ClearCache();
    }
    ...
    ImTextureID GetWallpaperTexture(const std::string& name) override {
        ...
        std::string path = modDir + "wallpaper/" + name;
    ...
    void Refresh() override {
        ...
        std::string modDir = GetModDirectory();
        std::string folderPath = modDir + "wallpaper/";
        try {
