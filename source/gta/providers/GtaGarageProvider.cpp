...
bool GtaGarageProvider::IsVehicleValid(CVehicle* vehicle) {
    if (!vehicle) return false;
    if (!CPools::ms_pVehiclePool) return false;
    return CPools::ms_pVehiclePool->IsObjectValid(vehicle);
}

bool GtaGarageProvider::IsPedValid(CPed* ped) {
    if (!ped) return false;
    if (!CPools::ms_pPedPool) return false;
    return CPools::ms_pPedPool->IsObjectValid(ped);
}

bool GtaGarageProvider::IsModelIdValid(int modelId) {
    if (modelId < 0) return false;
    return CModelInfo::GetModelInfo(modelId) != nullptr;
}

bool GtaGarageProvider::FindSafeSpawnLocation(float minDist, float maxDist, float& outX, float& outY, float& outZ, float& outHeading) {
...
bool GtaGarageProvider::IsInside() {
    int interior = 0;
    plugin::Command<COMMAND_GET_AREA_VISIBLE>(&interior);
    return interior > 0;
}

bool GtaGarageProvider::IsOnMission() {
    return CTheScripts::IsPlayerOnAMission();
}

bool GtaGarageProvider::IsPlayerInAnyCar() {
    CPlayerPed* player = FindPlayerPed(-1);
    if (!player) return false;
    return player->bInVehicle && player->m_pVehicle != nullptr;
}

bool GtaGarageProvider::GetCurrentVehicleInfo(VehicleInfo& outInfo) {
    CPlayerPed* player = FindPlayerPed(-1);
...
int GtaGarageProvider::TriggerDelivery(const VehicleInfo& info) {
...
unsigned int GtaGarageProvider::GetSpawnedVehicleHandle(int deliveryId) {
...
bool GtaGarageProvider::IsVehicleNearPlayer(unsigned int handle) {
...
bool GtaGarageProvider::IsPlayerInVehicleModel(int modelId) {
    CPlayerPed* player = FindPlayerPed(-1);
    if (!player || !player->bInVehicle || !player->m_pVehicle) return false;
    return player->m_pVehicle->m_nModelIndex == modelId;
}

void GtaGarageProvider::Update(float dt) {
...
void GtaGarageProvider::ShowMessage(const std::string& message, int durationMs) {
    // Disabled
}
