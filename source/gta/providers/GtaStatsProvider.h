...
public:
  ...
  PlayerStats GetPlayerStats() override {
    ...
    stats.stamina = CStats::GetStatValue(STAT_STAMINA) / 10.0f;
    stats.stamina = std::clamp(stats.stamina, 0.0f, 100.0f);

    stats.sexAppeal = CStats::GetStatValue(STAT_SEX_APPEAL) / 10.0f;
    stats.sexAppeal = std::clamp(stats.sexAppeal, 0.0f, 100.0f);
  ...
  GangStats GetGangStats() override {
    ...
    if (player && player->m_pPlayerData) {
      ...
      if (groupId < 8) {
        stats.recruitedMembersCount =
            CPedGroups::ms_groups[groupId]
                .m_groupMembership.CountMembersExcludingLeader();
      }
    }
    ...
    if (respectVal >= 800.0f)
      stats.maxRecruitsCount = 7;
    else if (respectVal >= 600.0f)
      stats.maxRecruitsCount = 6;
    else if (respectVal >= 400.0f)
      stats.maxRecruitsCount = 5;
    else if (respectVal >= 200.0f)
      stats.maxRecruitsCount = 4;
    else if (respectVal >= 100.0f)
      stats.maxRecruitsCount = 3;
    else if (respectVal >= 10.0f)
      stats.maxRecruitsCount = 2;
    else
      stats.maxRecruitsCount = 0;
  ...
  CrimeStats GetCrimeStats() override {
    CrimeStats stats;
    stats.peopleKilled =
        static_cast<int>(CStats::GetStatValue(STAT_PEOPLE_YOUVE_WASTED));
    stats.carsStolen =
        static_cast<int>(CStats::GetStatValue(STAT_NUMBER_OF_CARS_STOLEN));
    ...
    stats.bustedCount =
        static_cast<int>(CStats::GetStatValue(STAT_TIMES_BUSTED));
    stats.wastedCount =
        static_cast<int>(CStats::GetStatValue(STAT_NUMBER_OF_HOSPITAL_VISITS));
    stats.starsEvaded = static_cast<int>(
        CStats::GetStatValue(STAT_TOTAL_NUMBER_OF_WANTED_STARS_EVADED));
  ...
  ProgressStats GetProgressStats() override {
    ...
    stats.completionPercentage =
        std::clamp(stats.completionPercentage, 0.0f, 100.0f);

    stats.storyMissionsCompleted =
        static_cast<int>(CStats::GetStatValue(STAT_MISSIONS_PASSED));
    stats.storyMissionsTotal = static_cast<int>(
        CStats::GetStatValue(STAT_TOTAL_NUMBER_OF_MISSIONS_IN_GAME));
    if (stats.storyMissionsTotal == 0)
      stats.storyMissionsTotal = 98; // Fallback

    int attempts =
        static_cast<int>(CStats::GetStatValue(STAT_MISSION_ATTEMPTS));
    stats.missionsFailed = attempts - stats.storyMissionsCompleted;
    if (stats.missionsFailed < 0)
      stats.missionsFailed = 0;

    stats.cheatsUsedCount =
        static_cast<int>(CStats::GetStatValue(STAT_TIMES_CHEATED));
    stats.photosTaken =
        static_cast<int>(CStats::GetStatValue(STAT_PHOTOGRAPHS_TAKEN));

    stats.oystersCollected =
        static_cast<int>(CStats::GetStatValue(STAT_OYSTERS_COLLECTED));
    stats.oystersTotal =
        static_cast<int>(CStats::GetStatValue(STAT_TOTAL_OYSTERS));
    if (stats.oystersTotal == 0)
      stats.oystersTotal = 50;

    stats.horseshoesCollected =
        static_cast<int>(CStats::GetStatValue(STAT_HORSESHOES_COLLECTED));
    stats.horseshoesTotal =
        static_cast<int>(CStats::GetStatValue(STAT_TOTAL_HORSESHOES));
    if (stats.horseshoesTotal == 0)
      stats.horseshoesTotal = 50;

    stats.tagsSprayed =
        static_cast<int>(CStats::GetStatValue(STAT_TAGS_SPRAYED));
    stats.tagsTotal = 100;

    stats.snapshotsTaken =
        static_cast<int>(CStats::GetStatValue(STAT_SNAPSHOTS_TAKEN));
    stats.snapshotsTotal =
        static_cast<int>(CStats::GetStatValue(STAT_TOTAL_SNAPSHOTS));
    if (stats.snapshotsTotal == 0)
      stats.snapshotsTotal = 50;
