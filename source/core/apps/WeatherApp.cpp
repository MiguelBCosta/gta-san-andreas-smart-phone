...
int WeatherApp::getSimulatedTemp(WeatherTheme theme, float rain) {
    switch (theme) {
...
void WeatherApp::onDraw() {
    ...
    WeatherInfo current = m_weatherProvider->GetCurrentWeather();
    WeatherInfo next = m_weatherProvider->GetNextWeather();
    float rain = m_weatherProvider->GetRainIntensity();
    ...
    if (rain > 0.0f || current.theme == WeatherTheme::RAINY) {
        ...
        char rainPercent[32];
        sprintf_s(rainPercent, sizeof(rainPercent), "%.0f%%", rain * 100.0f);
